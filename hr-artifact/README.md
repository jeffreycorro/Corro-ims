...
`public/index.html` should be the Claude artifact HTML export. Current deploy is **build 2026-09-27a** (Pipeline ingest merges notes, matches a missing applicant id by email then phone then name, stores docs.resume, and accepts key-gated mergeInto — on top of 2026-09-26c (201 checklist: SSS, TIN, PhilHealth and Pag-IBIG count as On file when that government number is filled in — digits with dashes count, a blank or dash-only box does not — and the percentage, missing list, dashboard, filters and export use that without waiting for the next save; an attached scan stays, and a row already marked On file is not cleared because the number box is empty; editing a 201 file debounces the save, does not redraw every employee’s checklist on each click, and does not resend photo or e-signature images that already have a Drive link or already sit in Settings; the 201 header says Editing, Saving, Saved or Not saved — on top of 2026-09-26b (Daily Manpower overtime stays on the day it was typed: a new day starts with OT blank, changing the date does not copy the open grid's OT onto the next day, and an edit writes only that day's `daily/dYYYYMMDD` document; the attendance list follows date hired and separation date, so someone is on a day only from their hire date until the day before they are separated, and a saved row from before they were hired stays hidden unless that row already has overtime or a status other than Present — on top of 2026-09-26a (Daily Manpower restores blank overtime from Change History when the last recorded figure is a number and the cell is still blank; a zero already on the sheet and a later clear stay blank — on top of 2026-09-25b (201 checklist: Refusal of Government-mandated Deductions and the KASABUTAN waiver are one row, and Pipeline Import from email says why Pull is off — on top of 2026-09-25a (Leave and Cash Advance Drive uploads send the service-account JWT with `sub` set from `GOOGLE_DRIVE_DELEGATED_USER`, so new files use that Workspace mailbox's quota instead of the service account's full My Drive; `storageQuotaExceeded` / `uploadQuotaExceeded` name that quota and the env var — on top of 2026-09-24c (Approver Approve stamps Jeffrey's e-signature onto the employee-signed uploaded PDF or image when one is on file, keeps that scan as originalSignedLink, and makes the stamped file the official Open / Print; with no upload it still stamps the portal form — on top of 2026-09-24b printable HR forms stamp company e-signatures from Settings: Cassie on Daily Manpower / attendance prepared-by, Domingo on department-head and HR lines, Catherine on evaluated-by, Jeffrey on authorizing lines that are not waiting in the Approver queue; the employee’s own signature stays blank and never blocks print; a missing required company JPEG stops the print and names the person and the filename — on top of 2026-09-23b Leave and Cash Advance print and send-for-approval stamp the Department Head and Evaluated by e-signatures from Settings → HR e-signatures, and stop with a message naming the missing JPEG instead of printing a blank box; Approver Approve still stamps the CEO e-signature on Approved by / Final Approval — on top of 2026-09-23a Approver tab asks for a password before the queue or its actions are shown; a wrong password is rejected and the unlock lasts only for this page, the same way Motorpool's office passcode does — on top of 2026-09-22b Approver queue: after the evaluator signs, Leave, employment contracts, and cash advances wait as Evaluated — for approval until Jeffrey approves and his e-signature is stamped on the form — on top of 2026-09-22a Daily Report Monitoring / Daily Manpower lists active employees only: Separated, Resigned, Terminated, and AWOL stay off today's open sheet even when a saved roster snapshot still names them; past filed days keep their rows — on top of 2026-09-19a Payroll drops Separated / resigned / terminated / AWOL the same way Daily Manpower does; Leave and Cash Advance print Catherine A. Largo as Evaluated by; Leave/CA signed-copy uploads accept scans up to 80 MB — on top of 2026-09-18g 201 checklist: COE CCD / COE from employee / employee requirement checklist / KASABUTAN / NBI Clearance; N/A counts toward the percentage; Separations is N/A while the person is not Separated; a government-mandated-deduction refusal attachment completes SSS / Pag-IBIG / PhilHealth / TIN — on top of 2026-09-18f Attendance / Daily Manpower: a 201 Project-based or Regular status sticks instead of the stale separated-roster re-apply, master Position titles show when present, blank day status defaults to Present, and Mark all Rest day confirms before changing the visible list — on top of 2026-09-18e Leave / Cash Advance Import from Drive, 2026-09-18d Ask Stop talking, 2026-09-18c photo/file attach, chat states, and leave answers that include the Reason for Leave field, on top of 2026-09-18b / multiple exam-score rows on an applicant, on top of 2026-09-18a / Claude's three surgical patches on 2026-09-17a: one pending status that pays 0, bulk rest/pending buttons that only change selects until Save, and hosted-site Drive-off copy)))). If you replace it, keep the shim script tag in `<head>`.
...
| `GOOGLE_DRIVE_DELEGATED_USER` | **All contexts**, Functions | `jeffreycorro@corroconstruction.com`. Leave/CA uploads impersonate this mailbox so files use his Drive quota. Aliases: `GOOGLE_DRIVE_IMPERSONATE`, `GOOGLE_IMPERSONATE_USER`. |
...
6. Leave and Cash Advance uploads create the file as the Drive token's identity. Sharing a My Drive folder with the service account (Editor) does **not** spend that user's quota — the service account owns the new file, and this service account's My Drive is full (`storageQuotaExceeded` / `uploadQuotaExceeded`).
   ...
   **Preferred:** impersonate `jeffreycorro@corroconstruction.com`. That mailbox must own or have free space in the Leave / Cash Advance / 201 folders.
   ...
   4. On Netlify site `corcondev-hr`, set `GOOGLE_DRIVE_DELEGATED_USER` to `jeffreycorro@corroconstruction.com`. **Contexts: All** (Production, Deploy Previews, Branch deploys, and Local — not Production only). Include **Functions** so the Drive function can read it. Redeploy after saving.
...
Settings → **HR e-signatures** stores JPEG/PNG signatures. Attach on a person’s row, or attach several files at once and let the filename pick the slot. The same Attach control is on Daily Manpower.

| Slot | Person | Filename keywords | Stamped on |
| --- | --- | --- | --- |
| Cassie — Daily Manpower | Maria Trina Cassandra A. Moran, HR staff | `cassie`, `casey`, `cassandra`, `moran`, `manpower`, `attendance` | Daily Manpower **Prepared by**. Also company memos, minutes, 201 gap report, exam result and hiring papers where the line is HR staff. A file already stored as Prepared by still fills this line until a Cassie file is attached. |
| Approved by | Domingo C. Monte Jr., Department Head / HR Officer | `domingo`, `monte`, `hr-head`, `approved` | Leave **HR Department**, Cash Advance **Department Head**, clearance **Certified by**, NTE **Issued by**, disciplinary memo HR line, COE **Prepared by**, memo **Confirmed by**, contract **HR head** lines. |
| Evaluated by | Catherine A. Largo | `catherine`, `largo`, `evaluated`, `payroll` | Leave and Cash Advance **Evaluated by**. Payroll and payslip **Prepared by**. |
| Final approval | Jeffrey James M. Corro | `jeffrey`, `signatory`, `ceo`, `president` | Approver **Approve** on Leave, Cash Advance and employment contracts (password `032589`). At print time on COE **Noted by**, company memo **Conformed**, and payroll **Approved by** — those sheets do not wait in the Approver queue. |

The employee’s signature, acknowledgement, affiant, quitclaim, and Leave **Immediate Supervisor** lines stay blank. Witness and notary lines stay blank. Finance on Cash Advance still prints the red “Finance officer name — Settings” hint when that name is empty; there is no finance JPEG slot. Marian U. Corro’s administrative-head lines stay blank — there is no image slot for that office.

Print is blocked, with a message that names the person and the expected filename, when a required company signature is missing. It is not blocked for the employee the form is about.
