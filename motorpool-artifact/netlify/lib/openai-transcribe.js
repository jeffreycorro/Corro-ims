...
const DEFAULT_PROMPT =
  "Workplace dictation for Motorpool staff at a Philippine construction company in Cebu. " +
  "Speech may mix English and Filipino or Cebuano (Taglish). " +
  "Keep unit codes (DT-03, BH-06), plates, VRF numbers, job-order numbers, and part names as spoken. " +
  "Use ordinary yard wording. Do not add commentary.";
...
async function transcribeAudio(input) {
  ...
  for (let i = 0; i < chain.length; i += 1) {
    ...
    if (
      ...
      res.status >= 400 &&
      res.status < 500 &&
      !modelUnavailable(res.status, payload) &&
      !isLegacyWhisper(model)
    ) {
