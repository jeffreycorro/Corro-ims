...
1. Confirm the live Ask / footer BUILD is **2026-09-26 a** (hard refresh if a service worker or tab still shows `2026-09-25 a`).
...
14. **Suppliers:** on New VRF, open a line’s Supplier dropdown. The last choice is **+ Add new supplier…**. Add a name. It is selected on that line and appears on the other lines and on Fuel → bulk delivery. Adding the same name again (any capitalisation, or with a comma) selects the one already on the list. **FUEL RESERVE** and **MOTORPOOL INVENTORY** stay the built-in choices. Office → Data health → **Manage suppliers** can rename or remove a typo; an old VRF line keeps the name it was saved with.
