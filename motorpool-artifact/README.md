...
1. Confirm the live Ask / footer BUILD is **2026-09-21 c** (hard refresh if a service worker or tab still shows `2026-09-21 b`).
...
7. **VRF log:** newest form at the top (date, then VRF number). Subtitle is **Newest first**. A VRF sent for approval appears immediately as **Requested — FOR APPROVAL**. Opening it shows the attached photos. **Print / save as PDF** (and the log-row Print button) works with that VRF number before Jeffrey approves.
