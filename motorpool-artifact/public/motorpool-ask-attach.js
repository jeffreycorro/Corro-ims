/**
 * Photo / file attach helpers for Ask the log.
 * Loaded by claude-shim.js. Hold-to-talk stays on the mic / Space hold.
 */
(function (root, factory) {
  ...
  if (root) root.mpAskAttach = api;
  if (typeof window !== "undefined" && window) window.mpAskAttach = api;
  ...
  function ensureStyles() {
    ...
    if (document.getElementById("mp-ask-attach-styles")) return;
    var style = document.createElement("style");
    style.id = "mp-ask-attach-styles";
    style.textContent =
      "[data-mp-ask-input='1']{min-height:56px;resize:vertical}" +
      ".mp-ask-attach-row{display:flex;flex-wrap:wrap;gap:8px;align-items:center;" +
      "padding:0 var(--space-4,16px) var(--space-2,8px)}" +
      ".mp-ask-chip{display:inline-flex;align-items:center;gap:6px;padding:3px 8px;" +
      "border:1px solid var(--rule,#d3d5d5);border-radius:999px;font-size:12px;" +
      "background:var(--surface,#fff)}" +
      ".mp-ask-chip img{width:22px;height:22px;object-fit:cover;border-radius:3px}" +
      ".mp-ask-atts{display:flex;flex-wrap:wrap;gap:6px;margin-top:6px}";
